...
@login_required
def DeleteFile(request, docu_id):
    docu_data = Documents.objects.get(id=docu_id)
    docu_data.delete()
...
@login_required
def MyDocuments(request, file_type):
    Profile = UserProfile.objects.get(user_id=get_user_account(request))
    docu_data = Documents.objects.filter(user_id = Profile.id).filter(folder_type=file_type).order_by("-id")
    print("88888888888")
    print(Profile)
...
def save_file(request):
    ...
    Documents.objects.create(
        ...
        folder_type = request.POST['folder_type'],
    )
